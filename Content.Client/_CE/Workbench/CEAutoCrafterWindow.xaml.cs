...
public sealed partial class CEAutoCrafterWindow : DefaultWindow
{
    ...
    public event Action<CEWorkbenchUiRecipesEntry>? OnSelectRecipe;
    ...
    public CEAutoCrafterWindow()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        Sawmill = _log.GetSawmill("ce_autocrafter_window");
    ...
    public void UpdateState(CEWorkbenchUiRecipesState autoCrafterState)
    {
        if (_player.LocalEntity is null)
            return;

        _cachedState = autoCrafterState;
        ...
        var sortedRecipes = autoCrafterState.Recipes
            .OrderByDescending(e => e.Craftable)
            .ThenByDescending(e =>
            {
                if (!_prototype.TryIndex(e.ProtoId, out var recipe))
                    return 0;
            ...
            {
                if (!_prototype.TryIndex(e.ProtoId, out var recipe) ||
                    !_prototype.TryIndex(recipe.Category, out var category))
                    return string.Empty;
                return category.ID;
            });
        ...
        // If there's a selected recipe, show it
        if (autoCrafterState.SelectedRecipe is not null)
        {
            var selectedRecipeEntry = autoCrafterState.Recipes.FirstOrDefault(r => r.ProtoId == autoCrafterState.SelectedRecipe);
            if (_prototype.TryIndex(autoCrafterState.SelectedRecipe.Value, out var selectedRecipe))
            {
                RecipeSelect(selectedRecipeEntry, selectedRecipe);
            }
        }
    ...
    private void OnSelectPressed(BaseButton.ButtonEventArgs _)
    {
        ...
        OnSelectRecipe?.Invoke(_selectedEntry.Value);
    }
