...
public sealed partial class ImperialStoreSystem
{
    ...
    public void RefreshAllListings(ImperialStoreComponent component)
    {
    ...
    public HashSet<ImperialListingData> GetAllListings()
    {
        var allListings = _proto.EnumeratePrototypes<ImperialListingPrototype>();

        var allData = new HashSet<ImperialListingData>();

        foreach (var listing in allListings)
        {
            allData.Add((ImperialListingData)listing.Clone());
        }
    ...
    public bool TryAddListing(ImperialStoreComponent component, string listingId)
    {
        if (!_proto.TryIndex<ImperialListingPrototype>(listingId, out var proto))
        {
    ...
    public bool TryAddListing(ImperialStoreComponent component, ImperialListingData listing)
    {
    ...
    /// <returns>The available listings.</returns>
    public IEnumerable<ImperialListingData> GetAvailableListings(
        EntityUid buyer,
        HashSet<ImperialListingData>? listings,
        HashSet<ProtoId<ImperialStoreCategoryPrototype>> categories,
        EntityUid? storeEntity = null)
    {
        ...
        foreach (var listing in listings)
        {
            ...
            {
                var args = new ImperialListingConditionArgs(buyer, storeEntity, listing, EntityManager);
    ...
    public bool ListingHasCategory(ImperialListingData listing, HashSet<ProtoId<ImperialStoreCategoryPrototype>> categories)
    {
