...
/// </summary>
public sealed partial class CEVehicleSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CEVehicleComponent, BeforeDamageChangedEvent>(OnBeforeDamageChanged);
        SubscribeLocalEvent<CEVehicleComponent, UpdateCanMoveEvent>(OnVehicleUpdateCanMove);
    ...
    /// </remarks>
    private void OnBeforeDamageChanged(Entity<CEVehicleComponent> ent, ref BeforeDamageChangedEvent args)
    {
    ...
    private void OnVehicleUpdateCanMove(Entity<CEVehicleComponent> ent, ref UpdateCanMoveEvent args)
    {
        var ev = new CEVehicleCanRunEvent(ent);
        RaiseLocalEvent(ent, ref ev);
        if (!ev.CanRun)
            args.Cancel();
    }

    private void OnVehicleShutdown(Entity<CEVehicleComponent> ent, ref ComponentShutdown args)
    {
        TryRemoveOperator(ent);
    }

    private void OnVehicleGetAdditionalAccess(Entity<CEVehicleComponent> ent, ref GetAdditionalAccessEvent args)
    {
    ...
    public bool TrySetOperator(Entity<CEVehicleComponent> entity, EntityUid? uid, bool removeExisting = true)
    {
        ...
        if (uid != null)
        {
            ...
            var enterEvent = new CEOnVehicleEnteredEvent(entity, uid.Value);
        ...
        var setEvent = new CEVehicleOperatorSetEvent(uid, oldOperator);
    ...
    [PublicAPI]
    public bool TryRemoveOperator(Entity<CEVehicleComponent> entity)
    {
    ...
    public bool TryRemoveOperator(Entity<CEVehicleOperatorComponent?> operatorEntity)
    {
        ...
        if (!TryComp<CEVehicleComponent>(operatorEntity.Comp.Vehicle, out var vehicle))
            return true;
    ...
    /// </summary>
    public EntityUid? GetOperatorOrNull(Entity<CEVehicleComponent?> entity)
    {
    ...
    /// </summary>
    [PublicAPI]
    public bool HasOperator(Entity<CEVehicleComponent?> entity)
    {
    ...
    /// </summary>
    public bool CanOperate(Entity<CEVehicleComponent?> entity, EntityUid uid)
    {
        if (!Resolve(entity, ref entity.Comp))
            return false;
    ...
    /// </summary>
    public void RefreshCanRun(Entity<CEVehicleComponent?> entity)
    {
    ...
    private void UpdateAppearance(Entity<CEVehicleComponent> entity)
    {
        ...
        {
            _appearance.SetData(entity, CEVehicleVisuals.CanRun, inputMover.CanMove, appearance);
        }

        _appearance.SetData(entity, CEVehicleVisuals.HasOperator, entity.Comp.Operator is not null, appearance);
    }
}
